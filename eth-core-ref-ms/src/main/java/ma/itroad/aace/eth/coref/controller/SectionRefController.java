...
@RestController
@RequestMapping(value = "/sectionRefs", consumes = {MediaType.ALL_VALUE}, produces = MediaTypes.HAL_JSON_VALUE)
@ResponseBody
public class SectionRefController {

    @Autowired
    ISectionRefService sectionRefService;
    ...
    @GetMapping("/all")
    ResponseEntity<Page<SectionRefBean>> getAll(@RequestParam(defaultValue = "0") int page,
                                                @RequestParam(defaultValue = "10") int size, @RequestParam(defaultValue = "DESC") String orderDirection) {
        Pageable pageable = PageRequest.of(page, size);
        Page<SectionRefBean> response = sectionRefService.getAll(pageable);
        return ResponseEntity.ok().body(response);
    }

    @GetMapping("all/{lang}")
    ResponseEntity<Page<SectionRefEntityRefLang>> getAll(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, @PathVariable("lang") String lang, @RequestParam(defaultValue = "DESC") String orderDirection) {
        Page<SectionRefEntityRefLang> response = sectionRefService.getAll(page, size, lang, orderDirection);
        return ResponseEntity.ok().body(response);
    }
    ...
    public ResponseEntity<String> importFile(@RequestParam("file") MultipartFile file) {
        importDataService.setService(sectionRefService);
    ...
    public ResponseEntity<InputStreamResource> getFile(@PathVariable("lang") String lang) throws IOException {
        ...
        InputStreamResource file = new InputStreamResource(sectionRefService.load(lang));
    ...
    public ResponseEntity deleteSection(@PathVariable("id") Long id) {
        ...
        ErrorResponse response = sectionRefService.delete(id);
    ...
    public ResponseEntity deleteList(@RequestBody ListOfObject listOfObject) {
        ErrorResponse response = sectionRefService.deleteList(listOfObject);
        return ResponseEntity.status(response.getStatus()).body(response);
    }
    ...
    public ResponseEntity deleteInternationalisation(@PathVariable("id") Long id, @PathVariable("lang") String lang) {
        ...
        ErrorResponse response = sectionRefService.deleteInternationalisation(lang, id);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

   /* @PostMapping("/add")
    public void addSectionRef(@RequestBody SectionRefEntityRefLang sectionRefEntityRefLang) {
        sectionRefService.addSectionRef(sectionRefEntityRefLang);
    }*/

    @PostMapping("/add")
    public SectionRefEntityRefLang addSectionRef(@RequestBody SectionRefEntityRefLang sectionRefEntityRefLang) {
        sectionRefService.addSectionRef(sectionRefEntityRefLang);
    ...
    public SectionRefEntityRefLang findSection(@PathVariable("lang") String lang, @PathVariable("id") Long id) {
        return sectionRefService.findSection(id, lang);
    }
...
/*
    ...
    public void addInternationalisation(@PathVariable("lang") String lang,@RequestBody EntityRefLang entityRefLang) {
        sectionRefService.addInternationalisation(entityRefLang,lang);
    }*/
    ...
    public ResponseEntity getAllByCodeOrLabel(@PathVariable("value") String value, @PathVariable("lang") String lang, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size)  {
        ...
        return ResponseEntity.ok().body( sectionRefService.filterByCodeOrLabelProjection(value , PageRequest.of(page,size) , lang));
    }

//    @GetMapping("/{id}")
//    public ResponseEntity getById(@PathVariable("id") Long id){
//        return ResponseEntity.ok().body(sectionRefService.findById(id));
//    }

    @PostMapping("/import-valid")
    public ResponseEntity importValidFile(@RequestParam("file") MultipartFile file) {

        return sectionRefService.saveFromExcelWithReturn(file);
