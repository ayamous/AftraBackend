...
@RestController
@RequestMapping(value = "/agreementTypes", consumes = { MediaType.ALL_VALUE }, produces = MediaTypes.HAL_JSON_VALUE)
public class AgreementTypeController {

    @Autowired
    IAgreementTypeService iAgreementTypeService;
    ...
    @GetMapping("/all")
    ResponseEntity<Page<AgreementTypeBean>> getAll(@RequestParam(defaultValue = "0") int page,
                                                   @RequestParam(defaultValue = "10") int size,
                                                   @RequestParam(defaultValue = "DESC") String orderDirection) {
        Pageable pageable = PageRequest.of(page, size);
        Page<AgreementTypeBean> response = iAgreementTypeService.getAll(pageable);
        return ResponseEntity.ok().body(response);
    }

    @GetMapping("all/{lang}")
    ResponseEntity<Page<AgreementTypeLang>> getAll(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, @PathVariable("lang") String lang,  @RequestParam(defaultValue = "DESC") String orderDirection) {
        Page<AgreementTypeLang> response = iAgreementTypeService.getAll(page, size, lang, orderDirection);
        return ResponseEntity.ok().body(response);
    }

    @PostMapping("/import")
    public ResponseEntity<String> importFile(@RequestParam("file") MultipartFile file) {
        importDataService.setService(iAgreementTypeService);
    ...
    public ResponseEntity<?> importValidFile(@RequestParam("file") MultipartFile file) {
        ...
        return iAgreementTypeService.saveFromExcelAfterValidation(file);
    }
    ...
    public ResponseEntity<InputStreamResource> getFile(@PathVariable("lang") String lang) throws IOException {
        String filename = "AgreementType.xlsx";
        InputStreamResource file = new InputStreamResource(iAgreementTypeService.load(lang));
    ...
    public ResponseEntity<InputStreamResource> getFile() throws IOException {
        String filename = "AgreementType.xlsx";
        InputStreamResource file = new InputStreamResource(iAgreementTypeService.load());
        MediaType excelMediaType = new MediaType("application", "vnd.ms-excel");
    ...
    public ResponseEntity deleteAgreement(@PathVariable("id") Long id) {
        ...
        ErrorResponse response = iAgreementTypeService.delete(id);
    ...
    public ResponseEntity deleteList(@RequestBody ListOfObject listOfObject) {
        ErrorResponse response = iAgreementTypeService.deleteList(listOfObject);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    @PostMapping("/add")
    public AgreementTypeLang addAgreementType(@RequestBody AgreementTypeLang agreementTypeLang) {
        iAgreementTypeService.addAgreementType(agreementTypeLang);
    ...
    public EntityRefLang addInternationalisation(@PathVariable("lang") String lang, @RequestBody EntityRefLang entityRefLang) {
        iAgreementTypeService.addInternationalisation(entityRefLang,lang);
    ...
    public ResponseEntity deleteInternationalisation(@PathVariable("id") Long id, @PathVariable("lang") String lang) {
        ...
        ErrorResponse response = iAgreementTypeService.deleteInternationalisation(lang, id);
        return ResponseEntity.status(response.getStatus()).body(response);
    }

    @GetMapping("/find/{lang}/{id}")
    public AgreementTypeLang findAgreementType(@PathVariable("lang") String lang, @PathVariable("id") Long id) {
        return iAgreementTypeService.findAgreementType(id, lang);
    }
    ...
    public ResponseEntity getAllByCodeOrLabel(@PathVariable("value") String value, @PathVariable("lang") String lang, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size)  {
        if (value.equals(" ") || value == null|| value.equals(""))
           ...
           return ResponseEntity.ok().body( iAgreementTypeService.filterByCodeOrLabelProjection(value , PageRequest.of(page,size) , lang));

//        return ResponseEntity.ok().body( iAgreementTypeService.filterByCodeOrLabel(value , PageRequest.of(page,size) , lang));
    }

//    @GetMapping("/{id}")
//    public ResponseEntity getById(@PathVariable("id") Long id){
//        return ResponseEntity.ok().body(iAgreementTypeService.findById(id));
//    }
